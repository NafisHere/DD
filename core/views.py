...
def add_to_wishlist(request):
    if not request.user.is_authenticated:
        return JsonResponse({"login_required": True, "redirect_url": reverse("userauths:sign-in")}, status=401)
    ...
    context = {}
    wishlist_count_for_product = wishlist_model.objects.filter(product=product, user=request.user).count()
    if wishlist_count_for_product > 0:
        # already in wishlist, keep idempotent
        pass
    else:
        wishlist_model.objects.create(product=product, user=request.user)

    # return updated total wishlist count for the user so frontend can update badges
    total_wishlist_count = wishlist_model.objects.filter(user=request.user).count()
    return JsonResponse({"bool": True, "wishlist_count": total_wishlist_count})
...
def remove_wishlist(request):
    ...
    try:
        ...
        total_wishlist_count = updated_wishlist.count()
        context = {'w': updated_wishlist}
        data = render_to_string("core/async/wishlist-list.html", context, request=request)

        return JsonResponse({"data": data, "wishlist_count": total_wishlist_count})
